...
function renderVRPuzzleState() {
    ...
    for (let i = 0; i < totalPieces; i++) {
        ...
        slot.id = `slot-${i}`;
        // Slots dienen jetzt als Drop-Targets, nicht als Klick-Targets
    ...
    STATE.unplacedPieces.forEach((pieceIdx, index) => {
        ...
        piece.classList.add('scramble-piece', 'draggable'); // Neu: 'draggable' Klasse
        ...
        // NEUE Events für Drag-and-Drop (Greifen und Loslassen)
        // Diese werden durch A-Frame-Komponenten auf den Controllern ausgelöst.
        piece.addEventListener('mousedown', onPieceGrab); 
        piece.addEventListener('mouseup', onPieceDrop);
...
/**
 * Handler für das Greifen/Starten des Zugs (mousedown/triggerdown).
 */
function onPieceGrab(e) {
    e.stopPropagation();
    const piece = e.currentTarget;
    if (piece.getAttribute('visible') === 'false') return;
    
    // Nur greifen, wenn noch kein Teil ausgewählt ist
    if (!STATE.selectedPiece) {
        piece.classList.add('selected-piece');
        piece.setAttribute('material', `color:#ff0; shader:flat`); // Visuelles Feedback (Gelb)
        STATE.selectedPiece = piece;
        
        // Die tatsächliche Bindung des Teils an den Controller erfolgt in der A-Frame-Komponente (siehe unten).
        logEvent("piece_grabbed", { pieceIndex: parseInt(piece.dataset.index, 10) });
    }
}

/**
 * Handler für das Loslassen/Ablegen (mouseup/triggerup).
 */
function onPieceDrop(e) {
    e.stopPropagation();
    const piece = e.currentTarget;
    
    if (STATE.selectedPiece !== piece) return; // Nur das aktuell ausgewählte Teil ablegen
    
    const pieceIndex = parseInt(piece.dataset.index, 10);
    
    // 1. Visuelle Markierung entfernen
    piece.classList.remove('selected-piece');
    piece.setAttribute('material', `src:#puzzleTex; shader:flat; repeat:${1 / CONFIG.PUZZLE.COLS} ${1 / CONFIG.PUZZLE.ROWS}; offset:${(pieceIndex % CONFIG.PUZZLE.COLS) / CONFIG.PUZZLE.COLS} ${1 - (Math.floor(pieceIndex / CONFIG.PUZZLE.COLS) + 1) / CONFIG.PUZZLE.ROWS}`);

    // 2. Prüfe, ob das Teil über einem gültigen Slot losgelassen wurde
    // Wir benötigen das Ziel-Element (den Slot), über dem das Teil losgelassen wurde.
    // Dies wird durch eine benutzerdefinierte A-Frame-Komponente im HTML (z.B. ein "Drop-Event") geliefert.
    // Wir prüfen hier den Raycaster-Treffer beim Mouseup.
    
    // ANNAHME: Das Raycaster-Event beim Loslassen liefert den Treffer im Detail.
    const intersection = e.detail?.intersection; 
    let targetEl = null;

    if (intersection && intersection.els && intersection.els.length > 0) {
        // Findet den ersten intersected Slot
        targetEl = intersection.els.find(el => el.classList.contains('slot'));
    }

    if (targetEl) {
        const slotIndex = parseInt(targetEl.dataset.index, 10);
        
        // 3. Logik prüfen und platzieren
        if (pieceIndex === slotIndex && STATE.boardState[slotIndex] === null) {
            ...
            // Teil visuell entfernen und Slot aktualisieren
            piece.setAttribute('visible', 'false');
            targetEl.setAttribute('material', `src:#puzzleTex; shader:flat; repeat:${1 / CONFIG.PUZZLE.COLS} ${1 / CONFIG.PUZZLE.ROWS}; offset:${(slotIndex % CONFIG.PUZZLE.COLS) / CONFIG.PUZZLE.COLS} ${1 - (Math.floor(slotIndex / CONFIG.PUZZLE.COLS) + 1) / CONFIG.PUZZLE.ROWS}`);
            targetEl.setAttribute('animation__highlight', { property: 'material.color', from: '#0f0', to: '#fff', dur: 500, easing: 'easeInOutQuad' });
            ...
            logEvent("error", {
              ...
              slotIndex: slotIndex || 'none' // 'none' falls kein Slot getroffen wurde
            });
            targetEl.removeAttribute('animation__wrong');
            targetEl.setAttribute('animation__wrong', { property: 'material.color', from: '#888', to: '#f00', dur: 150, dir: 'alternate', loop: 1 });
            
            // Das Teil muss visuell durch die A-Frame-Komponente zum Scramble-Bereich zurückkehren.
        }
        
    } else {
      // Loslassen außerhalb eines Slots: Teil zurücksetzen (visuell durch A-Frame Komponente)
      logEvent("piece_dropped_out", { pieceIndex: pieceIndex });
    }
    
    // Auswahl auf null setzen, egal ob erfolgreich oder nicht
    STATE.selectedPiece = null;
}
